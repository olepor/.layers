...
(defun my-go/post-init-company ()
  (spacemacs|add-company-hook go-mode))

(defun my-go/init-company-go ()
...
(defun my-go/init-go-mode()
  ...
  (use-package go-mode
    ...
    :config
    (setq go-coverage-display-buffer-func 'display-buffer-reuse-window)
    (progn
      ...
      (defun spacemacs/my-go-run-tests (args)
        (interactive)
        (shell-command (concat "go test " args))
        (evil-delete-buffer "*Shell Command Output*") ;; TODO - not pretty.
        ;; Consider using start-process and filters instead of this hack
      ...
      (defun spacemacs/my-go-run-test-current-file-coverage ()
        (interactive)
        (when (file-exists-p (concat (file-name-sans-extension (buffer-name)) "_test.go"))
          (shell-command (concat "go test -coverprofile=coverage.out"))
          (evil-delete-buffer "*Shell Command Output*") ;; TODO - not pretty.
          ;; Consider using start-process and filters instead of this hack
          ;; for now, only compatible with evil FIXME
          (go-coverage "coverage.out")))

      ;; FIXME not tested!!
      (defun spacemacs/my-go-run-test-current-function-and-cover ()
        (interactive)
        (when (file-exists-p (concat (file-name-sans-extension (buffer-name)) "_test.go"))
          (save-excursion
            (re-search-backward "^func[ ]+\\(([[:alnum:]]*?[ ]?[*]?[[:alnum:]]+)[ ]+\\)?\\(Test[[:alnum:]_]+\\)(.*)")
            (shell-command (concat "go test " "-run=" (match-string-no-properties 2) "-coverprofile=coverage.out"))
            (evil-delete-buffer "*Shell Command Output*")
            (go-coverage "coverage.out"))))
      ...
      (spacemacs/set-leader-keys-for-major-mode 'go-mode
        ...
        "tf" 'spacemacs/my-go-run-test-current-file-coverage))))

(defun my-go/init-go-eldoc()
  (add-hook 'go-mode-hook 'go-eldoc-setup))

(defun my-go/init-go-guru()
